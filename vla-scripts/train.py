...
class TrainingConfig:
    ...
    run_root_dir: Path = Path("logs")                # Path to directory to store logs & checkpoints
...
def log_metrics_to_tensorboard(metrics, prefix, step, writer: SummaryWriter) -> None:
    ...
    Args:
        ...
        writer (str): Tensorboard SummaryWriter instance
    ...
    for name, value in metrics.items():
        ...
        if name == "loss_value":
            writer.add_scalar(f"{prefix}/Loss", value, step)
        # Keep other metrics as is
        else:
            writer.add_scalar(f"{prefix}/{name.replace('_', ' ').title()}", value, step)
...
def save_training_checkpoint(
    ...
    # Create directories and save dataset statistics
    ...
    print(f"Saved model for Step {log_step} at: {checkpoint_dir}")
...
def train(cfg: TrainingConfig) -> None:
    ...
    # Initialize tensorboard logging and create experiment run directory
    if distributed_state.is_main_process:
        print(f"Training OpenVLA Model `{cfg.vla_path}` on `{cfg.dataset_name}`")
        print('Config:')
        pprint(cfg)
        ...
        writer = SummaryWriter(cfg.tensorboard_log_dir / run_id, flush_secs=30, filename_suffix=f"{datetime.now()}")
    ...
    vla.language_model.model.gradient_checkpointing_enable({"use_reentrant": False})
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if distributed_state.is_main_process and gradient_step_idx % cfg.tensorboard_log_freq == 0:
